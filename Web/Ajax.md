...
자바스크립트를 이용한 비동기 통신이며, 클라이언트와 서버 간에 XML 데이터를 주고받는 기술이다.  
Ajax를 사용하면 전체 페이지를 새로고침 하지 않아도 뷰를 갱신할 수 있다.  

## 동작 방식
브라우저가 가진 XMLHttpRequest 객체를 이용해 데이터를 로드한다.  
이 객체는 서버로부터 XML 데이터를 전송받아 처리하는데 사용된다.  
단순한 텍스트 값만 넘길 수도 있고, 폼을 넘길 수도 있고, 파일 업로드를 할 수도 있다.
